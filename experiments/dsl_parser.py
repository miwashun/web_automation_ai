  ...
  "$defs": {
    "Step": {
      ...
      "oneOf": [
        ...
        {"properties": {"act": {"const": "wait"}, "for": {"type":"string"}, "timeout": {"type":"integer","minimum":0}}},
        {"properties": {"act": {"const": "wait_for"}, "selector":{"type":"string"}, "state":{"enum":["attached","detached","visible","hidden","enabled","disabled","stable"]}, "timeout":{"type":"integer","minimum":0}}},
        # ---- 成果物/検証（既存） ----
        {"properties": {"act": {"const": "screenshot"}, "target":{"enum":["fullpage","viewport","selector"]}, "path":{"type":"string"}, "record":{"type":"boolean"}}},
        {"properties": {"act": {"const": "wait_download"}, "pattern":{"type":"string"}, "to":{"type":"string"}}, "required": ["pattern"]},
        {"properties": {"act": {"const": "verify_file"}, "hash":{"enum":["sha256","sha1","md5"]}, "record":{"type":"boolean"}}},
